...
def load_pred(pred_masks_names, iter_idx, data_len):
    img_masks_names_dict = {}
    for idx in range(data_len):
        img_name = str(idx).zfill(5)+"_"+str(iter_idx).zfill(2)
        img_masks_names = [pred_masks_name for pred_masks_name in pred_masks_names if img_name in pred_masks_name]
...
def cal_acc(gt_data, img_masks_names_dict, level, iter_idx, t2i_out_dir, in_masks_folder):
    ...
    for idx in range(len(gt_data_per_level)):  # loop on samples
        ...
        img_name = str(idx + (level * mul)).zfill(5)+"_"+str(iter_idx).zfill(2)
...
if __name__ == "__main__":
    ...
    for iter_idx in range(iter_num):
        for level in range(3):
            # Load Predictions:
            img_masks_names_dict = load_pred(pred_masks_names=pred_masks_names, iter_idx=iter_idx, data_len=len(gt_data))
