...
class MMOCRInferencer(BaseMMOCRInferencer):
    ...
    def forward(self, inputs: InputsType) -> PredType:
        ...
        if self.mode == 'rec':
            ...
            result['det'] = self.textdet_inferencer(inputs, get_datasample=True, pred_score_thr=0.55)
            if self.mode.startswith('det_rec'):
                result['rec'] = []
                if len(result['det'][0].pred_instances.polygons) > 0:  # Eslam: Handle empty detections case
