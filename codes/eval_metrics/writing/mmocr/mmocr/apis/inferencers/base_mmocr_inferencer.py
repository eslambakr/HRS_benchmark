...
class BaseMMOCRInferencer(BaseInferencer):
    ...
    def __call__(self, user_inputs: InputsType,
        ...
        # Eslam: filter the predictions based on the conf
        from copy import deepcopy
        scores = []
        polygons = []
        if type(preds) is list:
            if hasattr(preds[0], 'pred_instances'):
                for i in range(len(preds[0].pred_instances.scores)):
                    if preds[0].pred_instances.scores[i].item() > kwargs['pred_score_thr']:
                        scores.append(preds[0].pred_instances.scores[i].item())
                        polygons.append(preds[0].pred_instances.polygons[i])
                temp_instancedata = InstanceData()
                temp_instancedata.scores = torch.as_tensor(scores)
                temp_instancedata.polygons = polygons
                preds[0].pred_instances = temp_instancedata
